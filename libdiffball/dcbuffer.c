...
void free_DCommand_collapsed(DCommand_collapsed *dcc)
{
	free(dcc->commands);
}
...
{
	if ((dcb->srcs = (DCB_registered_src *)realloc(dcb->srcs, sizeof(DCB_registered_src) * dcb->src_array_size * 2)) == NULL)
	{
		return MEM_ERROR;
	}
...
void DCB_full_get_next(CommandBuffer *buff, DCommand *dc)
{
...
void DCB_matches_get_next(CommandBuffer *buff, DCommand *dc)
{
...
void DCB_llm_get_next(CommandBuffer *buff, DCommand *dc)
{
	DCB_llm *dcb = (DCB_llm *)buff->DCB;
	assert(dcb->flags & DCB_LLM_FINALIZED);
	if (dcb->main == NULL)
...
void DCB_full_truncate(CommandBuffer *buffer, unsigned long len)
{
...
void DCB_matches_truncate(CommandBuffer *buffer, unsigned long len)
{
	off_u64 trunc_pos;
...
void DCB_llm_truncate(CommandBuffer *buffer, unsigned long len)
{
	off_u64 trunc_pos;
...
void DCB_full_incr(CommandBuffer *buff)
{
	((DCB_full *)buff->DCB)->command_pos++;
}

void DCB_matches_incr(CommandBuffer *buff)
{
	DCB_matches *dcb = (DCB_matches *)buff->DCB;
	assert(dcb->cur - dcb->buff < dcb->buff_size);
	dcb->cur++;
}
void DCB_llm_incr(CommandBuffer *buff)
{
	DCB_llm *dcb = (DCB_llm *)buff->DCB;
...
void DCB_full_decr(CommandBuffer *buff)
{
	((DCB_full *)buff->DCB)->command_pos--;
}

void DCB_matches_decr(CommandBuffer *buff)
{
	DCB_matches *dcb = (DCB_matches *)buff->DCB;
	if (dcb->cur != dcb->buff)
	{
...
void DCB_llm_decr(CommandBuffer *buff)
{
	DCB_llm *dcb = (DCB_llm *)buff->DCB;
...
int DCB_add_overlay(CommandBuffer *dcb, off_u64 diff_src_pos, off_u32 len, DCB_SRC_ID add_ov_id,
...
int DCB_no_buff_add_add(CommandBuffer *buffer, off_u64 src_pos, off_u32 len, DCB_SRC_ID src_id)
{
...
int DCB_full_add_add(CommandBuffer *buffer, off_u64 src_pos, off_u32 len, DCB_SRC_ID src_id)
{
	DCB_full *dcb = (DCB_full *)buffer->DCB;
...
int DCB_no_buff_add_copy(CommandBuffer *buffer, off_u64 src_pos, off_u64 ver_pos, off_u32 len, DCB_SRC_ID src_id)
{
	DCB_no_buff *dcb = (DCB_no_buff *)buffer->DCB;
	dcb->dc.type = (buffer->srcs[src_id].type & 0x1);
	dcb->dc.data.src_pos = src_pos;
...
int DCB_full_add_copy(CommandBuffer *buffer, off_u64 src_pos, off_u64 ver_pos, off_u32 len, DCB_SRC_ID src_id)
{
	unsigned long index;
	DCB_full *dcb = (DCB_full *)buffer->DCB;
...
int DCB_matches_add_copy(CommandBuffer *buffer, off_u64 src_pos, off_u64 ver_pos, off_u32 len, DCB_SRC_ID src_id)
{
	DCB_matches *dcb = (DCB_matches *)buffer->DCB;
...
int DCB_llm_add_copy(CommandBuffer *buffer, off_u64 src_pos, off_u64 ver_pos, off_u32 len, DCB_SRC_ID src_id)
{
	DCB_llm *dcb = (DCB_llm *)buffer->DCB;
	assert((DCB_LLM_FINALIZED & dcb->flags) == 0);
...
void DCB_matches_reset(void *dcb)
{
	((DCB_matches *)dcb)->cur = ((DCB_matches *)dcb)->buff;
}

void DCB_llm_reset(void *dcb)
{
...
int DCB_common_init(CommandBuffer *buffer, unsigned long buffer_size,
...
int DCB_no_buff_init(CommandBuffer *buffer, unsigned long buffer_size, off_u64 src_size, off_u64 ver_size, cfile *out_cfh)
{
	DCB_no_buff *dcb;
...
int DCB_full_init(CommandBuffer *buffer, unsigned long buffer_size, off_u64 src_size, off_u64 ver_size)
{
	DCB_full *dcb;
...
int DCB_matches_init(CommandBuffer *buffer, unsigned long buffer_size, off_u64 src_size, off_u64 ver_size)
{
	DCB_matches *dcb;
...
int DCB_llm_init(CommandBuffer *buffer, unsigned long buffer_size, off_u64 src_size, off_u64 ver_size)
{
	DCB_llm *dcb;
...
int cmp_llmatch(void *dl1, void *dl2)
{
...
int DCB_llm_finalize(void *d_ptr)
{
