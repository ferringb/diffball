...
// Copyright (C) 2013-2014 Brian Harring <ferringb@gmail.com>
#include <stdlib.h>
...
void relative_encoder_free(struct relative_encoder *p)
{
	if (p->last_directory)
	{
...
void fix_redundant_slashes(char *path)
{
	char *current = path;
...
int relative_encoder_encode_path(struct relative_encoder *p, const char *original_path, char **calculated_path)
{
...
int relative_encoder_decode_path(struct relative_encoder *p, const char *data, char **resultant_path)
{
	int parents_ignored = 0;
...
{
#define write_or_return(func, args...)  \
	{                                   \
		int err = (func)(patchf, args); \
		if (err)                        \
		{                               \
			ERETURN(err);               \
		};                              \
...
#define read_string_or_return(value)                      \
	{                                                     \
		(value) = (char *)cfile_read_null_string(patchf); \
		if (!(value))                                     \
		{                                                 \
			eprintf("Failed reading null string\n");      \
			ERETURN(PATCH_TRUNCATED);                     \
		};                                                \
	};
...
#define read_path_or_return(value)                                 \
	{                                                              \
		int err = relative_encoder_cread_path(pe, patchf, &value); \
		if (err)                                                   \
		{                                                          \
			eprintf("Failed reading path encoded string\n");       \
			ERETURN(err);                                          \
		};                                                         \
	};
...
#define read_or_return_fixed(value, len)                 \
	{                                                    \
		if ((len) != cread(patchf, buff, (len)))         \
		{                                                \
			eprintf("Failed reading %i bytes\n", (len)); \
			ERETURN(PATCH_TRUNCATED);                    \
		};                                               \
...
#define read_or_return_variable(value)                               \
	{                                                                \
		...
		if (tmp < 0)                                                 \
		{                                                            \
			eprintf("Failed reading variable int from the patch\n"); \
			ERETURN(PATCH_TRUNCATED);                                \
		};                                                           \
		(value) = tmp;                                               \
	}
